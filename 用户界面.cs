...
namespace CSharpSQL课设
{
    public partial class 用户界面 : Form
    {
        ...
        public void select()//查找用户所订票
        {
            dataGridView1.Rows.Clear();//清空旧数据
            directives directives = new directives();
            string sql = $"select 车次,出发地,出站时间,目的地,预计抵达时间,''d,车票价格,日期 FROM 车票预定信息表 where 用户名='{DATA.userid}' AND 身份证号='{DATA.usersfz}'";
                IDataReader dr = directives.Read(sql);
        ...
        private void button2_Click(object sender, EventArgs e)
        {
            ...
            else
            {
                ...
                {
                    string sql = $"exec inst '{DATA.userid}','{DATA.userxm}','{DATA.usersfz}'" +
                        $",'{dataGridView1.SelectedRows[0].Cells[7].Value}','{dataGridView1.SelectedRows[0].Cells[0].Value}'" +
                        $",'{dataGridView1.SelectedRows[0].Cells[1].Value}', '{dataGridView1.SelectedRows[0].Cells[2].Value}'" +
                        $",'{dataGridView1.SelectedRows[0].Cells[3].Value}', '{dataGridView1.SelectedRows[0].Cells[4].Value}'" +
        ...
        private void button3_Click(object sender, EventArgs e)
        {
            if (dataGridView1.SelectedRows.Count > 0)//选择了一项
            {
                string sql = "delete from 车票预定信息表 where 用户名='" + DATA.userid + "'";
                directives directives = new directives();
                int n = directives.Update(sql);
                if (n > 0)
                {
        ...
        private void button6_Click(object sender, EventArgs e)
        {
            ...
            if (textBox1.Text != "")
            {
                string sql = $"select * from 车次信息表 where 目的地 like '%{textBox1.Text}%'";
                directives directives = new directives();
                IDataReader reader = directives.Read(sql);
                while (reader.Read())
                {
                    this.dataGridView1.Rows.Add(reader[0].ToString(), reader[1].ToString(), reader[2].ToString(), reader[3].ToString(), reader[4].ToString(), reader[5].ToString(), reader[6].ToString(), reader[7].ToString());
                }
                reader.Close();
                directives.X();
            }
